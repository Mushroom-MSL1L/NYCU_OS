...
Kernel::Kernel(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // ppt page 7
            ...
            // next argument is Thread name
            // next next argument is priority
            ASSERT( i + 2 < argc ) ; 
            /* push job queue with argv[0]*/
            execfileNum += 1;
            execfile[execfileNum] = argv[i + 1];
            /* add the job priority as argv[1]*/
            execfilePriority[execfileNum] = atoi(argv[i + 2]);
            i += 2 ;

            /* if not debug z mode, just print like above "-e" */
            if (!debug->IsEnabled('z')) {
                cerr << execfile[execfileNum] << " with priority " << execfilePriority[execfileNum] << "\n";
            }
        }
    ...
    // use this implementation to sort 
    // or change scheduler.cc to use SortedList
    for (int i = 1; i <= execfileNum; i++) {
        for (int j = i + 1; j <= execfileNum; j++) {
            if (execfilePriority[i] < execfilePriority[j]) {
                int temp = execfilePriority[i];
                execfilePriority[i] = execfilePriority[j];
                execfilePriority[j] = temp;

                char* temp2 = execfile[i];
                execfile[i] = execfile[j];
                execfile[j] = temp2;
            }
        }
    }
...
int Kernel::Exec(char* name)
{
    ...
    /* HW4 implementation */
    t[threadNum]->setPriority(execfilePriority[threadNum]);
    // char buffer[1000] ;
    // debug->debug_z_message(buffer, "A", stats->totalTicks, name);
    // DEBUG(dbgZZZ, buffer);
    /* end of HW4 implementation */
// ---------
