...
/* HW4 implementation */
const char dbgZZZ = 'z';        // job scheduling
// in the begin of main, if -d z is set, then "job scheduling debug mode" is enabled
// after that, if we call "DEBUG(z, "message")", then the message will be printed to cerr
...
class Debug {
  public:
    ...
    /* HW4 implementation */
    // just a tool for debug message format
    // use polymorphism
    void debug_z_message(char* buffer, char *zType, int current_total_tick, char *first_process_name, char *second_process_name);
    void debug_z_message(char* buffer, char *zType, int current_total_tick, char *process_name);
    /* end of HW4 implementation */
